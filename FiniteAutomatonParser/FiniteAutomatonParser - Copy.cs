...
{
    public class FiniteAutomatonParser : IFiniteAutomatonParser
    {
        public Core.IClassTable ClassTable { private get; set; }

        public Core.INodeCollection Nodes { private get; set; }

        public IEnumerable<string> UnclassifiedTerminals { private get; set; }

        private IIndexer<string, int> TokenClassIndices => ClassTable.TokenClassesIndices;
        ...
        private int undefinedTokenNameId;
        private int unclassifiedTokenNameId;


        public IFiniteAutomatonParserResult Parse()
        {
            classesWithRemovedSymbols = new Dictionary<string, List<char>>();
            undefinedTokenNameId = TokenClassIndices.GetIndex(ClassTable.UndefinedTokenClassName);
            unclassifiedTokenNameId = TokenClassIndices.GetIndex(ClassTable.UnclassifiedTokenClassName);
            ...
            int tokenClassId)
        {
            ...
            if (terminalState.TokenNameId != undefinedTokenNameId && terminalState.TokenNameId != tokenNameId)
            {
                throw new Exception("Attempted assignment of second tokenName to one state.");
            }
            else
            {
                terminalState.TokenNameId = tokenNameId;
                terminalState.TokenClassId = tokenClassId;
            }
            ...
            (FiniteState nextState, int stateIn) CreateState()
            {
                ...
                nextState.Links = new Dictionary<string, int>();
        ...
        private void CreateStatesForToken(IMedium token, List<FiniteState> states, FiniteState rootState)
        {
            ...
            if (token is IFactor f && f.Recursion != null)
            {
            ...
            var parsedNodes = new Dictionary<IMedium, Dictionary<string, int>>();

            parsedNodes[token] = CreateFiniteStatesForTokenFromFactor(token, token, states, rootState, references, parsedNodes, (IDefinedToken)token);
        ...
        )
        {
            ...
            var nextStates = new Dictionary<string, int>();
            ...
            {
                if (currentState.TokenNameId != undefinedTokenNameId && currentState.TokenNameId != token.Id)
                {
                    throw new Exception("Attempted assignment of second tokenName to one state.");
                }
                else
                {
                    currentState.TokenNameId = token.Id;

                    currentState.TokenClassId = token.TokenClassId;
                }
            }
            ...
            foreach ((ITerminal nextNode, IFactor nextFactor) in terminals)
            {
                if (nextFactor == null)
                {
                    ...
                    CreateStatesForUnclassifiedTerminals(nextNode.Name, states, currentState, token.Id, token.TokenClassId);
                }
                else
                {
            ...
            foreach ((IClass nextNode, IFactor nextFactor) in classes)
            {
                ...
                if (nextFactor == null)
                {
                    ...
                    if (currentState.Links.ContainsKey(className))
                    {
                        ...
                        if (nextState.TokenNameId != undefinedTokenNameId && nextState.TokenNameId != token.Id)
                        {
                            throw new Exception("Attempted assignment of second tokenName to one state.");
                        }
                        else
                        {
                            nextState.TokenNameId = token.Id;
                        }
